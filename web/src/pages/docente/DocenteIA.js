...
export default function DocenteIA() {
  const [mensagens, setMensagens] = useState([
    {
      id: '1',
      tipo: 'ia',
      texto: 'Olá! Sou o assistente do Sistema JURAI. Posso ajudá-lo com informações sobre as suas bancas, currículo ou qualquer dúvida sobre o sistema. Como posso ajudar?'
    }
  ]);
  ...
  const sugestoes = [
    'Em que bancas participo este mês?',
    'Como actualizo o meu currículo?',
    'Quais são os meus cargos nas bancas?',
    'Como funciona o sistema JURAI?',
  ];
      ...
      <div className="max-w-4xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 flex flex-col flex-1">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">Assistente académico · UNIKIVI</p>
          </div>
        ...
        <div className="card flex-1 flex flex-col overflow-hidden" style={{ minHeight: '500px' }}>
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {mensagens.length === 1 && (
              <div className="mb-6">
                <p className="text-xs text-gray-400 mb-3 font-medium uppercase tracking-wider">Sugestões</p>
            ...
            {mensagens.map((msg) => (
              <div key={msg.id} className={`flex gap-3 ${msg.tipo === 'utilizador' ? 'justify-end' : 'justify-start'}`}>
                {msg.tipo === 'ia' && (
